<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            ...
            @can('create user')
                <a href="{{ route('user.create') }}" class="bg-slate-700 text-md rounded-md px-4 py-2 text-white">Create</a>
            @endcan
        </div>
    </x-slot>
                ...
                <tbody>
                    ...
                    @forelse ($user as $users)
                        <tr class="border-b hover:bg-gray-100">
                            ...
                            <td class="px-6 py-4 capitalize">{{ $users->name }}</td>
                            <td class="px-6 py-4 ">{{ $users->email }}</td>
                            <td class="px-6 py-4 capitalize ">{{ $users->roles->pluck('name')->implode(',') }}</td>
                            <td class="px-6 py-4 capitalize">
                                {{ \carbon\carbon::parse($users->created_at)->format('d M, Y') }}</td>
                            <td class="px-6 py-4">
                                <div class="flex space-x-3">
                                    ...
                                    @endcan
                                    @can('delete user')
                                        <button onclick="deletePremission({{ $users->id }})"
